<?php
    if(isset($_POST['submit'])){
        ...
        if(empty($nombre) || empty($apellido) || empty($email) || empty($pass)){
            header("Location: /santaelena?e=campo_vacio");
            exit();
        }
        else {
            // Nombre y Apellido
            if(!preg_match("/^[a-zA-Z]*$/", $nombre)){
                header("Location: /santaelena?e=nombre");
                exit();
            }
            if(!preg_match("/^[a-zA-Z]*$/", $apellido)){
                header("Location: /santaelena?e=apellido");
                exit();
            }
            // Email y Contraseña
            if(!filter_var($email, FILTER_VALIDATE_EMAIL)){
                header("Location: /santaelena?e=email");
                exit();
            }
            if($_POST['password'] != $_POST['password2'] || sizeof($_POST['password']) > 12){
                header("Location: /santaelena?e=pass_match");
                exit();
            }
            else{
                ...
                if($result_check ){ 
                    header("Location: /santaelena?e=user");
                    exit();
                }
            }
        ...
        if ($insert) {
            header("Location: /santaelena?e=reg_success");
            exit();
        } else {
